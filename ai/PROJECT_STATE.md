...
## Completed
- [x] 2026-07-26 — **Integrity pass + library utility** (user flagged "Man of Steel because of Christopher Nolan??" and a just-watched film still in suggestions): (1) suggest.mjs/suggest-tv.mjs now record what a person did ON the candidate (director/writer/story/actor; TV adds creator/ep), scale their weight by it (story 0.4, ep 0.6 — a story credit no longer carries full director affinity), and the why says it honestly ("has a story credit here — you rated …"); (2) client-side **seen guard**: suggestion files are stale snapshots by design, so the client re-filters every pool/theme-pick/spotlight at render against ratings+watchlist+old-films+watched, matched by tmdbId (via enrichment) and normalized title|year ±1 (curly quotes/accents/punctuation collapsed; title-only matching avoided so unseen remakes aren't hidden), with an honest "N hidden — already logged" note; engines got the same normalization + tmdbId seen-sets; (3) **Films tab services drawer** — the shared streaming filter now runs over the seen library ("which of my films are on Netflix"), with "N not on your services" counts; (4) **Worth a rewatch** shelf on Watch: 4.5★+ always, 4★ after two years, ranked score + fade bonus (0.3/yr cap 5), narrowed by services, argued by the user's own rating notes. Verified with 11 Playwright checks incl. a mutation test (inject watched entry → card provably disappears + note appears)

- [x] 2026-07-26 — **Refresh from a phone**: `refresh.yml` workflow_dispatch (medium choice + optional pool/people, commits + republishes, run summary shows the new pool's head) so suggestions can be regenerated from github.com or the mobile app with no laptop; owner-guarded, inputs via `env` + digit-checked. Pool sizes moved into `config.json.suggest` so a bigger pool survives the daily sync (`--flag` > config > default; an empty flag falls through to config rather than past it — that bug was caught in testing). Static site footer deep-links to the workflow. **Not** one-tap from the page: the site is public, so it can hold no dispatch credential
- [x] 2026-07-26 — **Suggestion pools, lenses, streaming filters**: the suggesters now write a deep pool instead of a shortlist (films 60 / TV 40 / books 40, `--pool=` and `--people=` flags, two-pass per-person cap: 2 in the headline, 5 across the pool; `head` in the JSON marks the boundary). 15 theme picks instead of 3. Client gained a shared pool layer — lens chips (runtime/era/deep cuts/critics agree), order + stable shuffle, show more/show all, and a "Tonight" spotlight that rotates on the date. Opt-in streaming filter: `canonService` collapses TMDB's per-reseller names, chips are built from the pool (capped at 24 by coverage), picks persist in `localStorage` (`config.json.services` seeds a fresh browser), nothing is filtered until a service is picked. The lenses and the service filter also run over the hand-ranked **Up next** queues via `FACTS[medium]` (queue facts come from enrichment), with the drawer promoted to the top of the tab since it governs queue + spotlight + suggestions; a filtered queue always states what it withheld and never renumbers ranks. Verified with two Playwright passes over the real app (34 pool checks + 20 queue checks)
